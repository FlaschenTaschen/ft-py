...
SKILL_NAME="flaschen-taschen-py"
SKILL_PATH="${SKILLS_DIR}/${SKILL_NAME}"
REPO_URL="${REPO_URL:-https://github.com/FlaschenTaschen/ft-py.git}"
...
if [[ -d "${SCRIPT_DIR}/${SKILL_NAME}" ]]; then
    # Install from local directory (use symlink)
    echo "Installing from local directory..."
    ln -s "${SCRIPT_DIR}/${SKILL_NAME}" "${SKILL_PATH}"
    echo -e "${GREEN}✓${NC} ${SKILL_NAME} skill symlinked successfully"
else
    # Clone from git repo
